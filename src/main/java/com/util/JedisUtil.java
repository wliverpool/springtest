...
/**
 * * Redis工具类,用于获取RedisPool,连接非集群的redis. * 参考官网说明如下： * You shouldn't use the
 * same instance from different threads because you'll have strange errors. *
 * And sometimes creating lots of Jedis instances is not good enough because it
 * means lots of sockets and connections, * which leads to strange errors as
 * well. A single Jedis instance is not threadsafe! * To avoid these problems,
 ...
 * statically, it is thread-safe. * JedisPoolConfig includes a number of helpful
 * Redis-specific connection pooling defaults. * For example, Jedis with
 * JedisPoolConfig will close a connection after 300 seconds if it has not been
 * returned.
 * 
 * @author wufuming
 */
public class JedisUtil {
	...
	// Redis服务器IP
	private static String ADDR = "192.168.122.129";

	// Redis的端口号
	private static int PORT = 6379;

	// 访问密码
	private static String AUTH = "liverpool";
	...
	public Jedis getJedis(String ip, int port, String auth) {
		...
		do {
			try {
				jedis = getPool(ip, port, auth).getResource();
